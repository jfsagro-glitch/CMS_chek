...
const MobileInspection: React.FC = () => {
  ...
  const handleUploadPhotos = async () => {
    ...
    setIsUploading(true);
    try {
      // Проверяем, работаем ли мы с демо данными
      const isDemoMode = inspection?.data?.inspection?.comment?.includes('Демо осмотр');
      
      if (isOnline && !isDemoMode) {
        // Пытаемся отправить на сервер только для реальных осмотров
        try {
        ...
        // Демо режим или офлайн - просто сохраняем фото
        if (isDemoMode) {
